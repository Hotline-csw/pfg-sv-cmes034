...
//-----------------------------------------------------------------------------
//   (Class-)Name:   CustomFeedbackCompleteCustomerOrder
//
//   Description:    
//
//
...
[Export("CustomFeedbackCompleteCustomerOrder", typeof(HomagGroup.FLS.Infrastructure.Framework.Contracts.ICommandUserExit))]
[PartCreationPolicy(CreationPolicy.NonShared)]
[Description("Manual feedback for complete customer order")]
[EnabledScript(true)]
public class CustomFeedbackCompleteCustomerOrder : UserExitCustomBase, HomagGroup.FLS.Infrastructure.Framework.Contracts.ICommandUserExit//, IPartImportsSatisfiedNotification // TODO MESSAGING
{
...
#pragma warning restore 0649
    ...
    private string _TaskName = "CustomFeedbackCompleteCustomerOrder";
    ...
    public void Execute(object parameter)
    {
        ...
        try
        {
            _Logger = LogHelper.GetLogger(Name, typeof(CustomFeedbackCompleteCustomerOrder));
            ...
            if (itemEnumerable != null)
            {
                using (var unitOfWork = _UnitOfWorkFactory.CreateUnitOfWork())
                {
                    foreach (var selectedItem in itemEnumerable.OfType<CustViewMasterManualFeedback>())
                    {
                        ...
                        var topProductionOrders = productionOrdersRepository.GetFirstOrDefault(
                                po => po.TopProductionOrderNumber == selectedItem.TopProductionOrderNumber);
                        
                        if (topProductionOrders != null)
                        {
                            var productionItem2 = productionItemsRepository.GetFirstOrDefault(pi => pi.ProductionOrderCode == topProductionOrders.Code);
                            
                            if(productionItem2 != null)
                            {
                                var productionItemsStepsDataRepository2 = unitOfWork.GetRepository<ProductionItemsStepsData>();
                                var productionItemsStepsData2 = productionItemsStepsDataRepository2.GetFirstOrDefault(
                                        pisr => pisr.ProductionItemCode == productionItem2.Code && pisr.ProductionStepCode == "ASSEM");                            
                        
                                if (productionItemsStepsData2 != null)
                                {
                                    productionItem2.InsertFeedbackFinishedGood( unitOfWork, _TaskName,"6010","",1, _Logger);
                                }
                            }
                        }
