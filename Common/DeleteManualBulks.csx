...
[Export("DeleteManualBulks", typeof(HomagGroup.FLS.Services.Common.Contracts.JobScheduling.Configuration.Tasks.Generic.IGenericTask))]
[PartCreationPolicy(CreationPolicy.NonShared)]
[Description("[01] Delete all manual bulks")]
[EnabledScript(true)]
public class DeleteManualBulks : GenericTaskBase, HomagGroup.FLS.Services.Common.Contracts.JobScheduling.Configuration.Tasks.Generic.IGenericTask
{
    [Import]
    ...
    private string _TaskName = "DeleteManualBulks";
    ...
    public override void Execute(IJobExecutionContext executionContext)
    {
        ...
        try
        {
            _Logger = LogHelper.GetLogger(executionContext.Area, typeof(DeleteManualBulks));
            ...
            using(IUnitOfWork unitOfWork = _UnitOfWorkFactory.CreateUnitOfWork())
            {
                ...
                var manualBulk = manualBulksRep.GetQueryable(false);
                
                if(manualBulk.Any())
                {
                    unitOfWork.BulkDelete(manualBulk);
        ...
        catch (Exception e)
        {
            ...
            _Logger.Error(ResourcesKeys.ErrorInUserExit("DeleteManualBulks"), null, e);
            throw;
        }
    }
